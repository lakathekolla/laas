...
class LaasManager {
    ...
    loadServers() {
        ...
        return [
            ...
            { id: 'vite', title: 'Vite', command: cfg.get('viteCommand', DEFAULT_SERVERS[1].command) },
            { id: 'ngrok', title: 'ngrok', command: cfg.get('ngrokCommand', DEFAULT_SERVERS[2].command) },
        ];
    }
    async save() {
    ...
    async deleteServer(id) {
        ...
        const ok = await vscode.window.showWarningMessage(`Delete "${def.title}"? It will be stopped if running.`, { modal: true }, 'Delete');
        if (ok !== 'Delete') {
            return;
        }
    ...
    spawnServer(def) {
        ...
        const pty = {
            ...
            catch { /**/ } closeEmitter.fire(); },
            handleInput: (d) => { if (d === '\x03') {
                try {
                    proc.kill('SIGINT');
                }
                catch { /**/ }
    ...
    registerCommands() {
        ...
        this.reg('laas._msg', async (...args) => {
            ...
            switch (msg.type) {
                ...
                case 'saveServer':
                    if (id) {
                        this.updateServer(id, msg.title, msg.command);
                    }
                    break;
                case 'addServer':
                    this.addServer(msg.title, msg.command);
                    break;
                case 'deleteServer':
                    if (id) {
                ...
                case 'showTerminal':
                    if (id) {
                        this.running.get(id)?.terminal.show(true);
                    }
                    break;
            }
        });
    ...
    updateStatusBar() {
        ...
        if (this.startBtn) {
            this.startBtn.color = any ? '#4ade80' : undefined;
        }
        if (this.outputBtn) {
            this.outputBtn.color = any ? '#66c2ff' : undefined;
        }
        if (this.stopBtn) {
            if (any) {
                this.stopBtn.backgroundColor = new vscode.ThemeColor('statusBarItem.warningBackground');
                this.stopBtn.show();
            }
            else {
                this.stopBtn.backgroundColor = undefined;
                this.stopBtn.hide();
            }
        }
    }
    ...
    async showPicker() {
        ...
        if (p.label === '$(play) Start All') {
            await this.startAll();
            return;
        }
        if (p.label === '$(stop) Stop All') {
            await this.stopAll();
            return;
        }
        if (p.label === '$(refresh) Restart All') {
            await this.restartAll();
            return;
        }
        const def = this.servers.find(s => p.label === `$(terminal) ${s.title}`);
        if (def) {
            this.running.get(def.id)?.terminal.show(true);
        }
    }
    dispose() {
...
function activate(ctx) {
    ...
    ctx.subscriptions.push(vscode.window.registerWebviewViewProvider(LaasWebviewProvider.viewId, provider, {
